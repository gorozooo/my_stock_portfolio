...
class Command(BaseCommand):
    help = "11:50 10銘柄生成（ログ出力版・通知は未接続）"

    def handle(self, *args, **kwargs):
        ts = now().strftime("%Y-%m-%d %H:%M:%S")
        self.stdout.write(f"[{ts}] start 11:50 picks")
        qs = StockMaster.objects.all().order_by("code")[:UNIVERSE_LIMIT]

        # 軽量にスコアだけ計算して上位10件のコードをログ
        items = []
        for row in qs:
        ...
        self.stdout.write(self.style.SUCCESS(f"[{ts}] done 11:50 picks"))
