...
def ensure_benchmark_master() -> None:
  ...
  既に存在するものは name / kind / symbol / currency / unit / is_active を軽く同期。
  """
  existing: Dict[str, BenchmarkMaster] = {
    bm.code: bm
    for bm in BenchmarkMaster.objects.all()
  }
  ...
  for bd in BENCHMARK_DEFS:
    if bd.code in existing:
      ...
      if bm.name != bd.name:
        bm.name = bd.name
        changed = True
      if getattr(bm, "kind", None) != bd.kind:
        bm.kind = bd.kind
        changed = True
      if bm.symbol != bd.symbol:
        bm.symbol = bd.symbol
        changed = True
      if getattr(bm, "currency", None) != bd.currency:
        bm.currency = bd.currency
        changed = True
      if getattr(bm, "unit", None) != bd.unit:
        bm.unit = bd.unit
        changed = True
      if bm.is_active != bd.is_active:
        ...
        changed = True
      if changed:
        bm.save(update_fields=["name", "kind", "symbol", "currency", "unit", "is_active"])
    else:
      BenchmarkMaster.objects.create(
        code=bd.code,
        name=bd.name,
        kind=bd.kind,
        symbol=bd.symbol,
