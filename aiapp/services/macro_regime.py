...
def _download_ohlcv(symbol: str, days: int = 365) -> pd.DataFrame:
  ...
  # aiapp.models.features の正規化をそのまま利用
  cfg = FeatureConfig()  # 値は使わないが make_features 内でOHLCVを揃えるため
  # make_features は指標も付与するので、一度通してから OHLCV 部分を抜き出す
  df_feat = make_features(df, cfg=cfg)
  out = df_feat[["Open", "High", "Low", "Close", "Volume"]].copy()
  return out
...
def sync_benchmark_prices(days: int = 365) -> None:
  ...
  for bm in masters:
    try:
      ...
      logger.exception("[sync_benchmark_prices] download error %s: %s", bm.code, e)
      continue
