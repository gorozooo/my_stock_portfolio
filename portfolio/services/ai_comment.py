...
def make_ai_comment(
    *,
    ...
    sys = (
        ...
        "プロの現場感で、短く（1〜2文）、砕けた口調で、人間味のある一言コメントを書く。"
        "絵文字は軽く添える（多すぎない）。"
        "入力には Regime/Score/PrevScore/AdoptRate/TopSectors が含まれる。"
        "PrevScore があれば前日比を自分で差分計算して触れる。"
        "注目セクターをひとつか二つ具体名で示す。"
        "過度な断定や投資助言は避け、相場の“温度感”を表現する。"
        "出力は一段落のみ（改行・箇条書きなし、専門用語の羅列禁止、最大230文字）。"
    )
    user = (
