...
def _build_today_plan(user, assets: Dict[str, Any], news_trends: Dict[str, Any]) -> Dict[str, Any]:
    """
    TODAY PLAN（今日やること）
    - ASSETS + NEWS からルール生成（再現性ファースト）
    - aiapp/services/home_today_plan.py が本体
    """
    try:
        from aiapp.services.home_today_plan import build_today_plan_snapshot  # type: ignore
        snap = build_today_plan_snapshot(user=user, assets=assets or {}, news_trends=news_trends or {})
        if not isinstance(snap, dict):
            return {"title": "TODAY PLAN", "status": "error", "error": "today_plan is not dict", "actions": []}
        snap.setdefault("title", "TODAY PLAN")
...
def home(request):
    ...
    # ★ TODAY PLAN は assets/news を入力にする
    today_plan = _build_today_plan(request.user, assets=assets, news_trends=news_trends)
