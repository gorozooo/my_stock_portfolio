...
def _card_from(tr: TrendResult, pol: Dict[str, Any], credit: int) -> Dict[str, Any]:
    ...
    name = (
        tr.name
        or Holding.objects.filter(user=tr.user, ticker=tr.ticker.upper()).values_list("name", flat=True).first()
...
def build_board(user, *, use_cache: bool = True) -> Dict[str, Any]:
    ...
    # 3️⃣ TrendResult → ポリシーごとにフィルタリング
    ...
    data: Dict[str, Any] = {
        "meta": {
            ...
            "live": True,
            "source_path": ["service", "trendresult", "policy"],
            "source_breakdown": {},
        },
        "theme": {
