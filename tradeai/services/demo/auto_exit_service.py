...
# - CLOSED にした建玉から LearningResult も自動作成/更新します。
# =========================================================
...
from tradeai.services.learning.result_service import sync_learning_result_for_closed_trade
...
def auto_close_demo_trades_for_user(user, max_hold_bars: int = DEFAULT_MAX_HOLD_BARS) -> dict[str, Any]:
    ...
    for trade in open_trades:
        ...
        sync_result = sync_learning_result_for_closed_trade(trade)
        if sync_result.get("snapshot_created"):
            learning_snapshot_created += 1
        if sync_result.get("result") is not None:
    ...
    return {
        ...
        "learning_snapshot_created": learning_snapshot_created,
    }