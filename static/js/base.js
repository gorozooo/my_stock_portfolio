// base.js（確認モーダル＋文字＋ネオン進捗バー 流れるバー版）
document.addEventListener("DOMContentLoaded", function() {
  ...
  if (modal) {
    ...
    btnCancel.addEventListener("click", () => {
      modal.style.display = "none"; okCallback = null;
    });
    ...
    modal.addEventListener("click", e => {
      if(e.target===modal){ modal.style.display="none"; okCallback=null; }
    });
  }

  /* ========================================
     ===== ローディング画面（文字＋流れるネオンバー） ===== */
  ...
  Object.assign(barContainer.style,{
    width:'80%', maxWidth:'400px', height:'6px',
    background:'rgba(0,255,255,0.1)',
    borderRadius:'3px', overflow:'hidden',
    boxShadow:'0 0 12px #0ff inset', position:'relative'
  });
  ...
  document.querySelectorAll("a[href]").forEach(link=>{
    link.addEventListener("click",e=>{
      ...
      e.preventDefault(); showLoading(()=>window.location.assign(href));
    });
  });
