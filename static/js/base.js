...
document.addEventListener("DOMContentLoaded", function() {
  ...
  if(canvas && canvas.getContext){
    ...
    for(let i=0;i<PARTICLE_COUNT;i++){
      particles.push({x:Math.random()*width,y:Math.random()*height,vx:(Math.random()-0.5)*0.5,vy:(Math.random()-0.5)*0.5,size:Math.random()*3+1,hue:Math.random()*360});
    }
  ...
  Object.assign(loadingOverlay.style,{
    ...
    background:'rgba(0,0,20,0.85)',display:'flex',
  ...
  Object.assign(barContainer.style,{
    ...
    borderRadius:'3px', overflow:'hidden', boxShadow:'0 0 12px #0ff inset',
    position:'relative'
  });
